#
# spec file for package rh-python36-python-PySocks
#
...
# Manually manage miscased upstream package names
Provides:       %{?scl_prefix}python-pysocks = %{version}-%{release}
Obsoletes:      %{?scl_prefix}python-pysocks <= %{version}-%{release}
Conflicts:      %{?scl_prefix}python-pysocks <= %{version}-%{release}
...
in which the proxy settings have not been changed via the `set_proxy` method.
